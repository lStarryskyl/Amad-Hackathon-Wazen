...
function extractGeneratedExports(src) {
  ...
  for (const m of src.matchAll(
    /^export\s+(?:const|function|type)\s+(\w+)/gm
  )) {
    names.add(m[1]);
  }
  return names;
...
function extractManualExports(src) {
  ...
  for (const m of manualSection.matchAll(/export\s+(?:type\s*)?\{([^}]+)\}/g)) {
...
const manualNames = extractManualExports(indexSrc);

const allCollisions = [...manualNames].filter((n) => generatedNames.has(n));
...
console.error(
  `\n[check-collisions] ✗ ${unexpected.length} unexpected export name collision(s) detected between ` +
    `generated/api.ts and the manual sections of index.ts:\n`
);
